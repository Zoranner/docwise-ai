...
| 属性 | 说明 |
|------|------|
| 状态 | 已归档（2026-04-22 起不再作为现行前端真值） |
| 关联文档 | 当前信息架构以 [`project-workspace-design.md`](./project-workspace-design.md) 为准；视觉体系以 [`visual-system-design.md`](./visual-system-design.md) 为准；产品与应用架构见 [`product-design.md`](./product-design.md) |
| 技术栈 | 历史草案语境：Nuxt 4、Nuxt UI、Tauri 2 |

---

> 归档说明
>
> 本文是 2026-04-14 的早期壳层草案，保留用于记录当时的 editor-first / IDE 式思路。
>
> 其中以下设定已经失效，不得继续作为实现依据：
>
> - 三列顺序为“左主导航 / 中主工作区 / 右统一对话”
> - `/app/*` 路由命名空间
> - “蓝图 / 任务 / 工作区” 作为左侧主导航主模型
> - 以 Nuxt UI 通用外观组件承接当前桌面界面视觉
> - “IDE 式工作区”作为当前产品主入口
>
> 当前前端真值已经收敛为：
>
> - 三列顺序：项目列表 / 项目对话 / 项目工作区
> - 主入口路由：`/projects`、`/projects/overview`、`/projects/:id`
> - 项目工作区一级标签：`蓝图 / 任务 / 日志`
> - 视觉实现路径：自研基础界面部件 + 主题 token，而非 Nuxt UI 主题承接

## 设计目标

- 将当前「单页试跑」提升为**可长期使用的桌面工作界面**：结构清晰、符合常见 IDE / 协作工具心智（左侧导航 + 中间工作区 + 右侧对话）。
- 与现有后端概念对齐：**工作区**、**蓝图**、**任务**、**文档与预览**、**统一对话框**、[`ActiveContext`](./product-design.md)（`workspaceId`、`filePath`、`blueprintId`、`taskId` 等）。
...
| `workspacePath` | **工作区根目录**（绝对路径）：与 [`product-design.md`](./product-design.md) 中 **Workspace** 一致；**主锚点**：`project.db`、`.agent`、默认资源树与蓝图落盘范围均以此为准；打开项目时调用 `workspace_open` |
...
- **项目中心**：次顶栏 **「列表 | 总览」**。**总览** 一屏聚合：各项目任务状态、待处理检查点、蓝图是否待批等（指标定义与交互细节见 [`archive-ui-agent-first-design.md`](./archive-ui-agent-first-design.md)「多项目并行与整体进度总览」）。
...
与 [`product-design.md`](./product-design.md) 中前端模块对应关系如下。
...
- **预览**：对应当前编辑器内容或已保存文件，调用现有 `preview_render`（或后续文件路径 + 后端读盘）；与 [`product-design.md`](./product-design.md) 场景四一致。
...
- **多项目宿主**：多 `ProjectContext`、前台切换、**跨项目总览**数据聚合（见 `archive-ui-agent-first-design.md`）。

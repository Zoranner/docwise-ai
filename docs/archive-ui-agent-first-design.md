...
| 属性 | 说明 |
|------|------|
| 状态 | 已归档（选型稿，2026-04-22 起不再与现行方案并列） |
| 关联文档 | 当前前端真值以 [`project-workspace-design.md`](./project-workspace-design.md) 与 [`visual-system-design.md`](./visual-system-design.md) 为准；产品架构见 [`product-design.md`](./product-design.md) |

---

> 归档说明
>
> 本文保留为“智能体优先”探索稿，不再作为现行选型面板。
>
> 文中 A / B / C 三种布局方案、全局编排主列优先、两栏壳层等内容，已经被后续“项目列表 / 项目对话 / 项目工作区”的三列项目化工作区方案覆盖。
>
> 若需要追溯设计分歧来源，可以继续阅读本文；若用于当前实现、评审或验收，请转到：
>
> - [`project-workspace-design.md`](./project-workspace-design.md)
> - [`visual-system-design.md`](./visual-system-design.md)
...
- 设计文档中的 **waiting_checkpoint**：在编排流里用 **高对比卡片** 呈现，**阻塞后续智能体步骤**直至用户回复或选择；与 [`docwise:checkpoint-changed`](./product-design.md) 事件对齐时，卡片状态与任务树节点同步。
...
- [`archive-ui-shell-design.md`](./archive-ui-shell-design.md) 描述的是 **强结构、IDE 友好** 的三列壳层，适合**熟手**与**精确点选**。

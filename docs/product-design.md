...
| 状态 | 定稿 |
| 文档类型 | 产品与应用架构设计 |
| 关联文档 | `mdweave` 文档预览与导出工具链设计；[`project-workspace-design.md`](./project-workspace-design.md)：前端信息架构标准；[`visual-system-design.md`](./visual-system-design.md)：前端视觉标准 |
...
本文中的 **Workspace** 仅指**文件系统工作区根目录**这一领域概念，不指代前端右侧那一列 UI。前端界面中的右侧区域若需表达该语义，应称为**项目工作区列**或**项目工作区界面**，避免与文件系统根目录混淆。
...
这里的“共享”指**单个项目内部共享同一条消息流**，不是指所有项目共用一个全局会话池。当前前端信息架构下，每个项目都有自己的统一对话框；跨项目切换时，前端应切换到目标项目自己的消息流，而不是把多个项目的消息混在一起。
...
当前前端原型不再采用 editor-first 的 `filePath + preview` 主模型，而是围绕项目焦点组织上下文。前端与 Tauri 状态同步时，`ActiveContext` 以项目及其结构化对象为主：
...
- `workspaceId` 表示当前文件系统工作区根目录
- `projectId` 表示当前前台项目
- `blueprintId`、`taskId`、`reviewId`、`outputId` 表示项目工作区内的当前焦点对象
- 当前前端原型中，焦点切换优先围绕 `Project -> Blueprint -> Task -> Review / Output` 这一链路展开

其中 `reviewId` 与 `outputId` 属于领域模型中的有效对象，但在当前前端信息架构里**不是右侧一级标签**。它们通常作为任务树或日志事件中的次级焦点出现，通过日志钻取、详情展开或后续细部面板承接，而不是与 `蓝图 / 任务 / 日志` 并列。

### 前端模块

当前前端实现基于 Nuxt 4 的 `app/` 目录，而不是旧的 `src/modules/` 结构。界面职责大致如下：

| 路径 / 模块 | 界面区域 | 主要功能 |
|-------------|----------|----------|
| `app/layouts/default.vue` | 应用总壳层 | 承接三列桌面壳层 |
| `app/components/project/ProjectRail.vue` | 第一列项目列表 | 项目切换、项目状态与进度摘要 |
| `app/components/shell/ConversationRail.vue` | 第二列项目对话框 | 当前项目连续消息流与输入区域 |
| `app/components/workspace/WorkspacePanel.vue` | 第三列项目工作区 | 承接 `蓝图 / 任务 / 日志` 三个一级标签 |
| `app/components/workspace/*.vue` | 工作区子面板 | 分别承接蓝图、任务树与事件日志 |
| `app/pages/projects/[id].vue` | 当前项目主页面 | 当前前台项目的主入口 |
| `app/pages/index.vue`、`app/pages/projects/index.vue` | 兼容跳转入口 | 跳转到当前项目主页面 |
| `app/pages/projects/overview.vue`、`app/pages/workspace/index.vue` | 历史兼容路由 | 当前仅保留跳转，不再承担主业务语义 |
...
因此 `/workspace` 在当前阶段不再表示独立“编辑器工作台”，而只是旧路由兼容层。
...
│   │   ├── workspace/            # 第三列项目工作区
│   │   └── desktop/              # 基础界面部件
│   ├── composables/              # 前端状态与数据拼装
│   ├── lib/                      # 标签、焦点与 mock 数据辅助
│   ├── types/                    # 前端类型定义
│   └── assets/css/               # 当前样式入口
└── src-tauri/
    ├── src/
    │   └── app/
    │       ├── workspace/        # 工作区管理、文件与根路径
...
| 2026-04-22 | 修正：补充前端标准文档引用；明确 `Workspace` 仅指文件系统根目录；前端实现现实改为 Nuxt `app/` 结构与项目化工作区三列壳层；`/workspace` 与 `/projects/overview` 标注为兼容跳转而非主业务入口 |
